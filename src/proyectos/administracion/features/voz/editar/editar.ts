...
@Component({
  selector: 'app-editar-voz',
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule],
  templateUrl: './editar.html',
  // styleUrls: ['./editar.scss'] // O comentado si usas el global
})
export class EditarVoz implements OnInit {
  ...
  ngOnInit(): void {
    this.idVoz = Number(this.route.snapshot.paramMap.get('id'));
    if (this.idVoz) {
  ...
  cargarDatosActuales(): void {
    ...
    this.vozService.getVoz().subscribe({
      next: (datos) => {
        const vozActual = datos.find(item => item.id_voz === this.idVoz);
        if (vozActual) {
          this.editarForm.patchValue({
            titulo: vozActual.titulo,
            categoria: vozActual.categoria,
            contenido: vozActual.contenido,
            fuente: vozActual.fuente
          });
          this.imagenActualUrl = vozActual.img;
          this.cargandoDatos = false;
        } else {
          // 2. Usamos el texto de error "no encontrado"
          this.errorMensaje = this.t().edicion_voz.error_no_encontrado;
          this.cargandoDatos = false;
        }
  ...
  onSubmit(): void {
    ...
    this.vozService.editarVoz(this.idVoz, payload).subscribe({
      next: (respuesta) => {
        console.log('Contenido de voz actualizado', respuesta);
        this.isSubmitting = false;
        this.volver();
      },
      error: (err) => {
        console.error('Error al editar el contenido de voz:', err);
        this.isSubmitting = false;
  ...
  volver(): void {
    this.router.navigate(['/administracion/voz']);
  }
}
